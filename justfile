...
# Compile playground UI (Tish → JS, legacy DOM).
# Clear CARGO_TARGET_DIR so the Tish workspace uses its own ./target (not this repo's).
build-app:
    cd "{{ TISH_ROOT }}" && env -u CARGO_TARGET_DIR cargo run -p tish --release -- compile "{{ justfile_directory() }}/app/main.tish" -o "{{ justfile_directory() }}/public/playground.js" --target js --jsx legacy

# Build browser VM WASM + wasm-bindgen glue into public/.
build-vm:
    ...
    wasm-bindgen "{{ TISH_ROOT }}/target/wasm32-unknown-unknown/release/tish_wasm_runtime.wasm" --out-dir "{{ justfile_directory() }}/public" --out-name tish_vm --target web
